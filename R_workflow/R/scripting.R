...
#' @keywords internal
build_osexp <- function(design_name, language, conditions_file, schema) {
  ...
  send <- c("send_trigger(var.target_word_trigger)",
...
export_opensesame <- function(stimuli, design, schema, outdir, base = NULL) {
  ...
  write_csv_utf8(loop_table(stimuli), file.path(outdir, csv_name))
  txt <- build_osexp(design$name, design$language, csv_name, schema)
