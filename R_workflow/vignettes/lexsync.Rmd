...
`win.callOnFlip`. The OpenSesame `.osexp` draws and shows the word inside an
`inline_script` and sends the marker immediately after `show()` returns, which
it does at the display refresh. Neither the matching nor the script generation
