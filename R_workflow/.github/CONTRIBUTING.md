<!-- Mirrored from the repository root's .github/CONTRIBUTING.md, which is the
canonical copy. pkgdown builds CONTRIBUTING.html from a file at the PACKAGE
root, and this package lives in a subdirectory of the monorepo, so the guide
has to exist here too for the R site's Community link to resolve. Keep the two
byte-identical below this comment, as templates/ and schema.yaml already are. -->
