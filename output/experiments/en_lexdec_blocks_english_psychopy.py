...
Design: en_lexdec_blocks    Language: english
Conditions file: en_lexdec_blocks_english_psychopy.csv
...
Run it as ``python this_file.py --participant 3`` (or with LEXSYNC_PARTICIPANT
set). The conditions file holds every counterbalancing list, and the run presents
the one list that participant is allocated, which is list 1 when no participant is
given.
...
CONDITIONS_FILE = os.path.join(HERE, "en_lexdec_blocks_english_psychopy.csv")
...
EVENTS = json.loads(r"""[{"type":"fixation","text":"+","ms":500},{"type":"text","field":"target","ms":800,"trigger":"@condition_trigger"},{"type":"response","keys":["f","j"],"timeout":2},{"type":"feedback","answer":"answer","correct":"Correct","incorrect":"Incorrect","no_response":"Too slow","ms":600,"blocks":["practice"]},{"type":"blank","ms":250}]""")
...
def participant_number(argv=None):
    """The participant this run is for: --participant N, else LEXSYNC_PARTICIPANT."""
    argv = list(sys.argv[1:]) if argv is None else list(argv)
    raw = os.environ.get("LEXSYNC_PARTICIPANT", "1")
    if "--participant" in argv:
        position = argv.index("--participant") + 1
        raw = argv[position] if position < len(argv) else raw
    try:
        return max(int(raw), 1)
    except ValueError:
        print("[lexsync] participant '%s' is not a number; presenting the first list."
              % raw)
        return 1
...
def trials_for_participant(trials, participant):
    """The one counterbalancing list this participant is presented.

    The conditions file carries every list, because the same file serves every
    participant. Running it whole would present each item once per list, which for a
    rotated design means each item once in every condition: exactly the repetition
    the counterbalancing exists to prevent. Participant p is given the pth list,
    cycling, which is the allocation lexsync's participant_table() writes.
    """
    lists = sorted({str(t.get("list", "1")) for t in trials}, key=list_key)
    if len(lists) < 2:
        return list(trials)
    wanted = lists[(participant - 1) % len(lists)]
    return [t for t in trials if str(t.get("list", "1")) == wanted]
...
def main():
    ...
    trials = trials_for_participant(load_trials(CONDITIONS_FILE), participant)
    print("[lexsync] participant %d: list %s, %d trials"
          % (participant, trials[0].get("list", "1") if trials else "-", len(trials)))
